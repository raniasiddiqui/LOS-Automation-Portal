...
class NavigationError(Exception):
    """
    Raised when a hop cannot be made.

    `environmental` separates a problem with the ENVIRONMENT (no test data, the
    site down, no credentials) from an app regression (a menu entry that has
    gone). The first is not a finding about the application and must never be
...
class Session:
    ...
    def __init__(self, run_id: str, mode: str = settings.VERIFY,
        ...
        # The pages adopt_page stepped away FROM, newest last. release_page
        # pops one — see both.
        self._left_behind: list = []
    ...
    def __enter__(self) -> "Session":
        ...
        # accept_downloads is stated rather than left to the default: the
        # Documents screen has a "Download Attachments" button, and a run that
        # silently discarded what it downloaded could not report whether the
        # button did anything. Files land in the run's own artifacts folder, so
        # what came back is evidence alongside the screenshots.
    ...
    def adopt_page(self, page) -> None:
        """
        Continue the run on another page of the same browser context.

        For a control that opens what it opens in a NEW TAB. Without this the
        session keeps driving the page the button was pressed on, which still
        shows the screen it always did — so the run reports that nothing
        happened while the thing it asked for is sitting in the tab next door.

        Everything bound to a page is rebound: the recorder, so the API calls
        the new page makes are the ones a failed-request check reads; the
        dialog and file-chooser handlers, so a native prompt cannot stall an
        unattended run; and the error sink. The old page is left open — it is
        evidence, and closing it can navigate the context.

        The page stepped away from is remembered, because a run that follows a
        tab has to be able to come back to the one the application lives on.
        See release_page.
        """
        if page is None or page is self._page:
            return
        self._left_behind.append(self._page)
        self._page = page
        page.set_default_timeout(settings.STEP_TIMEOUT_MS)
        page.on("dialog", lambda d: d.dismiss())
        page.on("filechooser", lambda fc: None)
        page.on("pageerror", lambda e: self.console_errors.append(str(e)[:300]))
        if self.recorder is not None:
            page.on("request", self.recorder.on_request)
            page.on("response", self.recorder.on_response)

    def release_page(self, close: bool = True) -> str:
        """
        Go back to the page the last adopt_page stepped away from.

        The other half of adopt_page, and a run that follows a tab without it
        is left driving that tab for the rest of its life. The risk rating is
        why: 'Perform Risk Rating' opens a scoring model that is a DIFFERENT
        application on a different host, and the session that followed it there
        never came back. Everything on the model worked and was reported as
        working — and then the verification leg asked that tab for My Bucket,
        got a blank page and nought rows, and called a case that plainly exists
        missing. Every check passed and the run was still an ERROR.

        The adopted tab is closed on the way out. It has been screenshotted by
        then, and leaving it open makes it something a later hop can follow by
        accident.

        Returns the URL resumed, or "" if there was nothing to go back to — in
        which case the session keeps driving what it has, which is no worse
        than before it was asked.
        """
        if not self._left_behind:
            return ""
        previous = self._left_behind.pop()
        if previous is None or previous.is_closed():
            return ""
        leaving = self._page
        self._page = previous
        # Deliberately NOT re-bound: this page's handlers and its recorder have
        # been attached since it was made. Binding them again would dismiss
        # every dialog twice and record every request twice.
        if (close and leaving is not None and leaving is not previous
                and not leaving.is_closed()):
            try:
                leaving.close()
            except PWError:
                pass
    ...
    def other_pages(self) -> list:
        """Pages of this context that are not the one being driven."""
        try:
    ...
    def navigate(self, target: Target) -> None:
        ...
        for i, step in enumerate(target.steps, start=1):
            ...
            try:
                ...
                status = R.ERROR if e.environmental else R.FAIL
    ...
    def _step_menu(self, step: NavStep) -> str:
        ...
        if not nav.get("ok"):
            raise NavigationError(
                ...
                f"{str(nav.get('error', ''))[:120]}", environmental=True)
    ...
    def _step_row_by_id(self, step: NavStep) -> str:
        ...
        if not wanted:
            raise NavigationError("No record id was given to look for.", environmental=True)
    ...
    def _match_label(self, items: list[dict], label: str, key: str = "label"):
        ...
        text ('Relationship with Ot...'), and builds spell things differently
    ...
    def read_grids(self) -> list[dict]:
        ...
        to add rows — these are linkage tables and there are 13 of
