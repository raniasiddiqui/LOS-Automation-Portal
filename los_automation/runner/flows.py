...
The flow this implements, end to end:
    ...
    All Obligors -> Create Obligor
      -> fill Basic Information, Save            (the obligor now exists)
      -> fill the tabs that unlock after saving
...
The last step is the reason the whole thing is worth automating: it closes the
loop. Anything typed in step one is re-read from the app through a completely
...
BASIC_INFORMATION: list[Field] = [
    ...
    Field("Obligor Id Type", "dropdown", optional=True, conditional=True),
...
class FlowResult:
    ...
    def to_dict(self) -> dict:
        d = {k: v for k, v in self.__dict__.items() if k != "checks"}
...
def create_obligor(headless: Optional[bool] = None, dry_run: bool = True,
                   ...
                   fill_tabs: bool = True) -> FlowResult:
    """
    Create an obligor, raise a credit application against it, then verify.
    ...
    try:
        with Session(run_id, mode=settings.TRANSACT, headless=headless,
            ...
            nav = cr.navigate_in_app(
                ...
                s.recorder)
            if not nav.get("ok"):
                raise NavigationError("Could not open All Obligors.", blocked=True)
            ...
            for spec in BASIC_INFORMATION:
                ...
                # too, or one awkward optional field aborts the whole create.
                    ...
                    if spec.conditional and _not_on_screen(exc):
                        say(f"  - {spec.label}: not rendered yet — the app "
                            f"shows it only once a related field is set")
                        continue
                    shot = s.screenshot(f"fill-failed-{spec.label[:24]}")
                    status = R.BLOCKED if spec.optional else R.FAIL
                    res.checks.append(R.Check(
                        name=f"Basic Information accepts {spec.label}",
                        status=status,
                        expected=f"{spec.label} can be set",
                        actual=str(exc),
                        detail="Filling stopped here rather than saving a "
                               "partial record."
                        if not spec.optional else "",
                        evidence=[shot] if shot else [],
                        screen="Basic Information"))
                    step(f"set {spec.label}", status=status, note=str(exc)[:160])
                    if not spec.optional:
                        say(f"  !! {spec.label}: {exc}")
            ...
            if dry_run:
                ...
                res.checks.append(R.blocked(
                    ...
                    "was created. Re-run without --dry-run to commit."))
                return _finish(res, s, say)
            ...
            if cid:
                ...
                res.checks.append(R.Check(
                    name="Obligor is saved",
                    status=R.BLOCKED,
                    detail="Save reported no error and named no failing rule, "
                           "but no Customer ID appeared, so it cannot be "
                           "confirmed the record was created. "
            ...
            if rid:
                ...
                res.checks.append(R.Check(
                    name="Raising the transaction creates a credit case",
                    status=R.BLOCKED,
                    detail="Proceed reported no error but no request ID could "
                           "be read from the page, so the case will be looked "
                           "up by obligor name instead.",
                    evidence=[s.screenshot("09-no-request-id")]))
            ...
            try:
                ...
                res.checks.append(R.blocked(
                    "The new case appears in My Bucket", str(e),
                    evidence=[s.screenshot("09-case-not-found")]))
                step("open the case from My Bucket", R.BLOCKED, str(e)[:160])
                return _finish(res, s, say)
            ...
            if not wait_for_case_menu(s, say):
                res.checks.append(R.blocked(
                    ...
                    evidence=[s.screenshot("10-no-case-menu")]))
                return _finish(res, s, say)
            try:
            ...
            res.checks.extend(verify_entered_values(s, res.entries, say))
            return _finish(res, s, say)
        ...
        res.blocked_reason = str(e)
        res.checks.append(R.blocked("Reach the Create Obligor form", str(e)))
    except Exception as e:  # noqa: BLE001 - environment failure, not a defect
        res.blocked_reason = str(e)[:400]
        res.checks.append(R.blocked("Run the create flow", str(e)[:400]))
...
#             FSD calls these Linkage Tables; the live application has
#             EIGHTEEN of them across these tabs, thirteen on Additional
#             Information alone.
#
...
def set_field(f: W.Filler, spec: Field, name_override: str = "") -> W.Entry:
    ...
    """
    return f.set_value(spec.label, name_override or spec.value, when=spec.when)
...
def fill_tab(s: Session, tab: TabSpec, dry_run: bool, say, step,
    ...
    if tab.skip:
        ...
        result.checks.append(R.blocked(f"{tab.title} is filled", tab.note))
        step(f"skip {tab.title}", R.BLOCKED, tab.note[:120])
        return []
    ...
    for spec in tab.fields:
        ...
        # unchecked, which is a far worse outcome than one recorded blocker.
            ...
            if not spec.optional:
                failed_required += 1
            result.checks.append(R.Check(
                name=f"{tab.title}: {spec.label} can be set",
                status=status,
                expected=f"{spec.label} accepts a value",
                actual=str(exc),
                detail="" if spec.optional else
                       "This field is marked mandatory, so the tab may not save.",
                screen=tab.label))
            say(f"    !! {spec.label}: {str(exc)[:110]}")
    ...
    if dry_run:
        ...
        if tab.kind == LINKAGE:
            f._close_modal()
        result.checks.append(R.blocked(
    ...
    if not f.entries:
        ...
        result.checks.append(R.Check(
            ...
            detail=f"Save raised no error, but {failed_required} mandatory "
                   f"field(s) could not be set, so whether it was stored "
                   f"is unclear. The round trip below is the real answer.",
            evidence=[shot], screen=tab.label))
    else:
        result.checks.append(R.passed(
            f"{tab.title} is saved",
...
def fill_obligor_tabs(needle: str, headless: Optional[bool] = None,
    ...
    try:
        ...
        res.blocked_reason = str(e)
        res.checks.append(R.blocked("Reach the obligor", str(e)))
    except Exception as e:  # noqa: BLE001
        res.blocked_reason = str(e)[:400]
        res.checks.append(R.blocked("Run the tab-fill flow", str(e)[:400]))
...
def find_case(s: Session, needle: str, say, max_pages: int = 12) -> str:
    ...
    nav = cr.navigate_in_app(
        ...
        s.recorder)
    if not nav.get("ok"):
        raise NavigationError("Could not open My Bucket.", blocked=True)
...
def verify_case(request_id: str, entries: list[dict],
    ...
    try:
        with Session(run_id, mode=settings.VERIFY, headless=headless,
            ...
            if not wait_for_case_menu(s, say):
                res.blocked_reason = ("The case opened but its sidebar never "
                                      "populated.")
                res.checks.append(R.blocked("The case menu loads",
                                            res.blocked_reason))
                return _finish(res, s, say)
            try:
                note = s._step_context_menu(
...
def verify_entered_values(s: Session, entries: list[dict],
                          say) -> list[R.Check]:
    """
    ...
    for screen, items in by_screen.items():
            ...
            try:
                ...
                for e in items:
                    checks.append(R.Check(
                        name=f"{screen}: {e['label']} carried through",
                        status=R.BLOCKED,
                        detail=f"The case has no reachable {screen!r} tab, so "
        ...
        for e in items:
            ...
            if _appears_in(typed, page_text):
                ...
                # either way. Saying so is the honest answer — reporting FAIL
                # here manufactured dozens of findings out of values that were
                # simply too short to look for.
                checks.append(R.Check(
                    name=name, status=R.BLOCKED,
                    detail=f"{typed!r} is too short to search for reliably, "
                           f"and {label!r} has no labelled field on the case's "
                           f"{screen}"
                           + (f" or in the {opened} saved row(s) opened here"
                              if opened_detail else "")
                           + ", so whether it carried across cannot be told "
                             "from this screen.",
                    evidence=[shot] if shot else [], screen=screen))
            else:
...
def _persist(res: FlowResult) -> str:
    ...
    compat = {
        "run_id": res.run_id,
        "target_key": "obligor.create",
        "target_title": ("Create an obligor and raise a credit application"
