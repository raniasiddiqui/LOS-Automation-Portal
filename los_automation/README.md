...
Phase 2 creates a case. This fills **twenty** screens inside a case that
already exists in My Bucket, and then proves the data actually stuck. With
*Create New Obligor* above them, that is the twenty-one things the portal
offers:
...
My Bucket -> find the case -> open it
  ...
  -> Collaterals        ask for a collateral by classification and name ->
                        Proceed -> fill every tab it opens, Saving each
  -> Facility Coverage  associate a collateral with a facility from BOTH
                        trees: '+' -> Collateral Association -> Save
  -> Financials         New Statement -> the chart of accounts, then the
                        variance, peer and analysis tabs
  -> Risk Rating        Perform Risk Rating -> Edit -> Generate Score ->
                        check the Calculation Sheet, Save
  -> Credit Memorandum  write all thirty-three narrative editors, Save
  -> eCIB Details       '+ Add' with a typed dialog, then '+ Upload' with a
                        PDF of an SBP CIB report
  -> Policies & Exceptions
                        raise a policy exception through Add Exception, Save
  -> Conditions         raise a condition, attach a file through the panel's
                        paperclip, Save
  ...
  -> PR Checklist       Perform PR -> fill every factor -> Generate, THEN Save
  -> History            read-only: does a link respond, does a row navigate
  -> re-open the case FROM MY BUCKET and compare every value against what
     was typed
```

The order above is the order a run fills them in, and two places in it are
load-bearing rather than cosmetic. **Facility Coverage** associates a facility
with a collateral, so it has nothing to associate until both exist.
**Financials** comes before **Risk Rating** because the rating model reads the
case's financials — score it first and it scores an empty case. And **History**
goes last: one of its checks clicks a row of Requests History, which navigates
to a *different* transaction, so it cannot sit in front of screens that expect
to still be on this one.

**History is the only read-only screen.** It writes nothing, so a dry run and a
live run of it do exactly the same thing, and the portal says so rather than
warning about data it is not going to write.
...
python -m los_automation.runner.cli --fill-case collaterals --case-id 52224-2026
python -m los_automation.runner.cli --fill-case coverage --case-id 52224-2026
python -m los_automation.runner.cli --fill-case financials --case-id 52224-2026
python -m los_automation.runner.cli --fill-case risk_rating --case-id 52224-2026
python -m los_automation.runner.cli --fill-case credit_memorandum --dry-run
python -m los_automation.runner.cli --fill-case ecib_details --case-id 52224-2026
python -m los_automation.runner.cli --fill-case policies --dry-run
python -m los_automation.runner.cli --fill-case conditions --dry-run
...
python -m los_automation.runner.cli --fill-case pr_checklist --case-id 52224-2026
python -m los_automation.runner.cli --fill-case litigation --dry-run
python -m los_automation.runner.cli --fill-case history --case-id 52224-2026
python -m los_automation.runner.cli --fill-case all --show
```

In the portal they are the twenty **Check** buttons under *Fill and check a
case's screens*, plus one that runs all twenty. The dry run / live choice is a
radio above them, and a live run needs the tick-box as well.

`--fill-case crmd_note` is the screen the application now calls **RMG Memo**.
The label changed; the key did not, because it is what the CLI, the portal's
button and every previous run's artifacts are filed under.
...
### RMG Memo (CRMD Note) is eight of the same thing
